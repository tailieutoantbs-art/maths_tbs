...
export default function GameJoinLobbyPage() {
  ...
  const handleEnterArena = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      // Chuẩn hóa định dạng PIN tự động viết hoa và loại bỏ TBS-
      const formattedPin = pin.toUpperCase().trim();
      const normalizedPin = formattedPin.replace('TBS-', '');
      
      if (formattedPin.startsWith('TBS-') || formattedPin.length === 6 || normalizedPin.length === 6) {
        showToast('success', '🚀 Kết nối Đấu trường thành công! Đang tiến vào phòng chờ...');
        router.push(`/games/play/${normalizedPin}?name=${encodeURIComponent(studentName)}`);
      } else {
