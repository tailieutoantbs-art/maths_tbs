...
export default function GameSetupWorkshop() {
  ...
  // Đăng ký lắng nghe thực tế số lượng người chơi kết nối vào sảnh chờ
  React.useEffect(() => {
    if (!generatedPin) {
      setLivePlayerCount(0);
      return;
    }
    const pinCode = generatedPin.replace('TBS-', '');
    let unsubscribe: () => void = () => {};

    const listenRoom = async () => {
      const { doc, onSnapshot } = await import('firebase/firestore');
      const { db } = await import('@/lib/firebase');
      unsubscribe = onSnapshot(doc(db, 'game_rooms', pinCode), (snapshot) => {
        if (snapshot.exists()) {
          const data = snapshot.data();
          setLivePlayerCount(data.players?.length || 0);
        }
      });
    };
  ...
  // Xử lý xuất bản Game và sinh Mã PIN thật trên Firestore
  const handlePublishGame = async () => {
    ...
    try {
      const { collection, getDocs, query, orderBy, doc, setDoc, serverTimestamp } = await import('firebase/firestore');
      const { db } = await import('@/lib/firebase');

      // Tải câu hỏi từ ngân hàng để trích xuất
      const qSnapshot = await getDocs(query(collection(db, 'cauhoi_nganhang'), orderBy('createdAt', 'desc')));
      ...
      if (allQuestions.length === 0) {
        showToast('error', 'Ngân hàng đề thi đang trống! Vui lòng thêm câu hỏi trước.');
        setIsPublishing(false);
        return;
      }

      // Trích xuất ngẫu nhiên số lượng câu hỏi đã chọn
      const shuffled = [...allQuestions].sort(() => 0.5 - Math.random());
      const selected = shuffled.slice(0, selectedQuestions);

      // Sinh mã PIN ngẫu nhiên 6 chữ số
      const pinCode = Math.floor(100000 + Math.random() * 900000).toString();

      // Lưu trữ phòng chơi thật lên Firestore
      await setDoc(doc(db, 'game_rooms', pinCode), {
        pin: pinCode,
        title: gameName,
        mode: gameMode,
        targetClass: targetClass,
        duration: Number(countdownTimer),
        status: 'waiting',
      ...
      setGeneratedPin(`TBS-${pinCode}`);
      showToast('success', 'Đã khởi tạo phòng thi đấu thực tế thành công! 🚀');
    } catch (error) {
      console.error("Lỗi khởi tạo phòng thi đấu:", error);
      showToast('error', 'Lỗi kết nối máy chủ Firebase.');
    } finally {
      setIsPublishing(false);
    }
  };
                ...
                <div className="bg-slate-900 rounded-3xl p-6 shadow-lg border border-slate-800 animate-fadeIn relative overflow-hidden">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-slate-800 rounded-xl p-4 text-center border border-slate-700">
                      ...
                      <p className="text-2xl font-black text-white">{livePlayerCount} <span className="text-xs font-normal text-slate-500">HS</span></p>
                    </div>
                    <button 
                      onClick={() => router.push(`/games/host?pin=${generatedPin.replace('TBS-', '')}`)}
                      className="bg-rose-600 hover:bg-rose-700 text-white rounded-xl p-4 text-xs font-black uppercase tracking-widest transition-colors shadow-lg shadow-rose-900/50 cursor-pointer"
                    >
                      BẮT ĐẦU <br/> GAME ⏭️
                    </button>
                  </div>
