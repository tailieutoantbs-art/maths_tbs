...
import { doc, setDoc, getDoc, onSnapshot, updateDoc, serverTimestamp, collection, getDocs, query, orderBy } from 'firebase/firestore';
...
export default function GameHostPage() {
  ...
  // 1. Khởi tạo phòng chơi và tải câu hỏi từ ngân hàng hoặc từ cấu hình phòng đã có
  useEffect(() => {
    const initHost = async () => {
      ...
      if (pinParam) {
        // Giáo viên khởi chạy từ phòng đã cấu hình sẵn ở Setup
        setPinCode(pinParam);
        try {
          const roomRef = doc(db, 'game_rooms', pinParam);
          const roomSnap = await getDoc(roomRef);

          if (roomSnap.exists()) {
            const roomData = roomSnap.data();
            
            // Lấy danh sách câu hỏi cấu hình trong phòng chơi
            if (roomData.questions && roomData.questions.length > 0) {
              setQuestions(roomData.questions);
            } else {
              // Fallback: nếu chưa có câu hỏi, lấy toàn bộ từ ngân hàng đề
              ...
              setQuestions(qList);
              await updateDoc(roomRef, { questions: qList });
            }

            setPlayers(roomData.players || []);
            setGameStatus(roomData.status || 'waiting');
            setCurrentQuestionIndex(roomData.currentQuestionIndex || 0);
            setShowAnswer(roomData.showAnswer || false);
            setLoading(false);
          } else {
            alert(`Không tìm thấy cấu hình phòng chơi [${pinParam}]!`);
            window.location.href = '/games/setup';
          }
        } catch (error) {
          console.error("Lỗi khi đồng bộ cấu hình phòng chơi:", error);
          alert('Lỗi đồng bộ dữ liệu từ Firestore.');
        }
      } else {
        // Tự động sinh phòng chơi mới ngẫu nhiên (Mặc định)
        ...
        try {
          ...
          console.error("Lỗi khởi tạo phòng đấu trường ngẫu nhiên:", error);
        }
      }
    };
