...
export default function QuestionBankPage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Kết nối trực tiếp vào luồng lưu trữ tài liệu nội bộ qua biến môi trường
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex gap-2">
              <button 
                ...
                className="px-5 py-2.5 bg-emerald-100 text-emerald-700 font-bold rounded-xl hover:bg-emerald-200 text-xs uppercase shadow-sm transition-colors flex items-center gap-2"
              >
                📊 Kết Quả Khảo Sát
              </button>
              <button 
                onClick={() => router.push('/dashboard')} 
                className="px-5 py-2.5 bg-slate-100 text-slate-600 font-bold rounded-xl hover:bg-slate-200 text-xs uppercase shadow-sm transition-colors flex items-center gap-2"
              >
                ⬅ Về Dashboard
              </button>
            </div>
