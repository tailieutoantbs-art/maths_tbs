...
} from '\var\task\toan-tbs-platform/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '\var\task\toan-tbs-platform/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('\var\task\toan-tbs-platform')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '\var\task\toan-tbs-platform',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('\var\task\toan-tbs-platform\.netlify\dist\run\handlers\server.js')
